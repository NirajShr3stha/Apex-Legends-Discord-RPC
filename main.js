...
/* final */
const Translation = require('./final/Translation');
const Gallery = require('./final/Gallery');
const Termination = require('./final/Termination');
...
/* Main:STEAM */
logger.info(`Apex Legends Discord RPC v${appVersion}`, 'main');
logger.info('Authenticating...', 'main:steam');
SteamClient.logOn({
...
SteamClient.on('error', function(e) {
    if (e.eresult == 5) {
        return logger.error('Failed to login, Wrong details.', 'main:steam');
    }

    return logger.error(`Failed to login, with code: ${e.eresult}`, 'main:steam');
});

SteamClient.on('steamGuard', async function(domain, callback) {
    const response = await prompts({ type: 'text', name: 'code', message: 'Enter your Steam Guard Code: ' });
    if (!response || !response.code) {
        SteamClient.logOff();
        return logger.error('Steam Guard code not entered.', 'main:steam');
    }
